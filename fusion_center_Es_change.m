...
E_s = power_noise_ratio.*N0;

fa = 0.05; % FA vector values taken from the paper based on th : 10^-4 - 6 W
%th = linspace(10^-4,6,length(fa));
[th,CW_p] = MAP_est_Es_change(fa,N0,E_s);
...
p_fa_ideal_arr =[];
for k=1:length(E_s)
[x,x_det] = stage1_ED_Es_change (nSU,nCodeWords,nSamples,E_s(k),fa);
CW = x;
...
u = (1./(E_s(k) + N0)).*x_p; %E[h^2] is 2 sigma^2 i.e 2*1/2 = 1. Also ||x||^2 is E_s
...
grid on
 ...
 hold on
 plot(power_noise_ratio_db,p_fa_MAP_mmse_final,'r--','LineWidth',2);
 xlabel('$P/N_0$ (dB)','FontSize',12,'FontWeight','bold','Color','k','Fontname', 'Arial','Interpreter', 'latex')
 ylabel('Probablity of false alarm ($P_{fa}$)','FontSize',12,'FontWeight','bold','Color','k','Fontname', 'Arial','Interpreter', 'latex')
 legend('LS Estimate', 'MMSE Estimate','Location','northwest','FontSize',10,'Fontname','Arial','Interpreter','latex');
 title('MAP combiner','FontSize',12,'FontWeight','bold','Color','k','Fontname', 'Arial','Interpreter', 'latex')
